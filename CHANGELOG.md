...
### Changed
- **`{j}` is now the single canonical form everywhere — no more `<join at="N">`**: The extractors (`read_file_section`, `read_multi_pointer_entries`, `read_quest_table`, `npc.extract_npc_dialogue_data`, `quest.extract_quest_file_data`) now produce `{j}`-separated text directly and carry a new `sub_offsets: list[int]` field on every entry dict listing the absolute pointer-slot address of each sub-string (length 1 for single-string entries, length > 1 for grouped entries). `rebuild_section`, `resolve_indexes_against_entries`, and `resolve_offsets_with_groups` read `sub_offsets` directly via a new `_entry_sub_offsets` helper instead of parsing offsets back out of the text with `parse_joined_text`. The old dual representation (`<join at="N">` internally, `{j}` on disk) collapses into one, so the export boundary no longer does a join rewrite — `export._to_csv_form` only runs the color-code transform now. The legacy `<join at="NNN">` form is still *accepted* on the import side for backward compatibility with pre-1.6.0 translation files: `_JOIN_SPLIT_RE` matches both forms (widened to `-?\d+` as a safety rail), `parse_joined_text` still reads the embedded offsets out of legacy CSV text, and `_entry_sub_offsets` falls back to it when given a hand-built entry dict without `sub_offsets`. `join_codes_to_csv` is kept as a deprecated no-op alias that rewrites any residual legacy tag in its input. This cleanly handles the non-contiguous-offsets case (e.g. a 4-slot grouped entry with `[A, 0, 0, B]` where `A` lives at slot 0 and `B` at slot 12): `sub_offsets` records the real slot addresses, and `rebuild_section` rewrites exactly the right slots without ever reading the join tags. Validated against `inf/quests` on a real `mhfdat.bin`: 2839 entries extract and a grouped 8-slot quest round-trips through the full index-keyed pipeline with every sibling pointer moving to the translation.

...
### Added
- **Binary fingerprint in index-keyed JSON metadata**: Index-keyed JSON exports now record a 16-char SHA-256 prefix of the decrypted/decompressed source binary in `metadata.fingerprint`. At import time the importer recomputes it on the target file and warns loudly on mismatch — catches the case where a translation extracted from one game version is being applied to a different version (or to a binary that already has translations applied). The warning does not abort the import. Foundation for future per-version `headers.json` support without committing to any particular versioning architecture.
